...
resource "aws_subnet" "pub-subnet1" {
  ...
  cidr_block  = "10.11.1.0/24" # 10.11.1.0 ~ 10.11.1.255
...
resource "aws_subnet" "pub-subnet2" {
  vpc_id      = aws_vpc.vpc_stage.id
  cidr_block  = "10.11.2.0/24" # 10.11.2.0 ~ 10.11.2.255
  map_public_ip_on_launch = true
...
resource "aws_subnet" "prv-subnet1" {
  vpc_id      = aws_vpc.vpc_stage.id
  cidr_block  = "10.11.3.0/24" # 10.11.3.0 ~ 10.11.3.255
...
resource "aws_subnet" "prv-subnet2" {
  vpc_id      = aws_vpc.vpc_stage.id
  cidr_block  = "10.11.4.0/24" # 10.11.4.0 ~ 10.11.4.255
  map_public_ip_on_launch = false # private subnet은 해당 설정값을 false로 둔다
  availability_zone = "us-east-1b"
  tags = {
    Name = "prv-subnet2"
...
resource "aws_route_table" "private-route-table" {
  ...
  route {
    cidr_block = "0.0.0.0/0"
    nat_gateway_id = "${aws_nat_gateway.stage-natgw.id}"
  }
  tags = {
...
#   nat_gateway_id              = aws_internet_gateway.stage-igw.id
# }
# resource "aws_route" "private-route" {
#   route_table_id              = aws_route_table.private-route-table.id
#   destination_cidr_block      = "0.0.0.0/0"
#   nat_gateway_id              = aws_internet_gateway.stage-natgw.id
# }
